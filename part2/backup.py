...
breadcrumbs = []
back_up_date = time.strftime("%Y%m%d-%H%M%S")
backup_file = f"breadcrumbs_{back_up_date}_.json.gz"
...
def run_one_day():
    ...
    def callback(message):
        ...
        try: # Load the message data as JSON.
            ...
            message.ack() # Acknowledge the message
            return
        breadcrumbs.append(payload)  # Store the payload for backup.
        ...
        message.ack() # Acknowledge the message
        # Track the progess by printing a message every 100,000 messages received.
        if state["received_count"] % 100000 == 0:
    ...
    with pubsub_v1.SubscriberClient() as subscriber: # subscriber client to connect to Pub/Sub.
        ...
        try:
            ...
            pass
    # Serialize once so we can report size before and after compression.
    json_bytes = json.dumps(breadcrumbs).encode("utf-8")
    uncompressed_size_mb = len(json_bytes) / (1024 * 1024)

    with gzip.open(backup_file, "wb", compresslevel=6) as f:
        f.write(json_bytes)
        compressed_ts = time.time()
    compressed_size_mb = os.path.getsize(backup_file) / (1024 * 1024)
    ...
    active_span = (compressed_ts - first_ts) if first_ts is not None else 0.0
    ...
    print(f"Compressed back up file at   : {_fmt_ts(compressed_ts)}")
    print(f"Size of back up file before compression: {uncompressed_size_mb:.2f} MB).")
    print(f"Size of back up file after compression: {compressed_size_mb:.2f} MB).")
